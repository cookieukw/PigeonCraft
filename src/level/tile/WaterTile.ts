...
export class WaterTile extends Tile {
  ...
  public override render(screen: Screen, level: Level, x: number, y: number): void {
    let col = Color.get(5, 5, 115, 115);
    let transitionColor1 = Color.get(3, 5, 211, 322);
    let transitionColor2 = Color.get(3, 5, 440, 550);
    
    let u = level.getTile(x, y - 1) && !level.getTile(x, y - 1).connectsToWater;
    let d = level.getTile(x, y + 1) && !level.getTile(x, y + 1).connectsToWater;
    let l = level.getTile(x - 1, y) && !level.getTile(x - 1, y).connectsToWater;
    let r = level.getTile(x + 1, y) && !level.getTile(x + 1, y).connectsToWater;
  ...
  public override getMapColor(): number { return Color.get(-1, 5, 5, 5); }
