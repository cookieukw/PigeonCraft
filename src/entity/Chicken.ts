...
export class Chicken extends Mob {
  ...
  public override render(screen: Screen): void {
    ...
    if (this.dir > 1) {
      ...
      xt += 4;
      if (Math.floor(this.walkDist / 8) % 2 === 1) xt += 2;
    }
    const xo = Math.floor(this.x - 8);
    const yo = Math.floor(this.y - 11);
    let col = Color.get(-1, 210, 540, 554);
    ...
    screen.render(xo + (flip1 * 8), yo + 0, xt + 640, col, flip1);
    screen.render((xo + 8) - (flip1 * 8), yo + 0, xt + 1 + 640, col, flip1);
    if (!this.isSwimming()) {
      screen.render(xo + (flip2 * 8), yo + 8, xt + 672, col, flip2);
      screen.render((xo + 8) - (flip2 * 8), yo + 8, xt + 1 + 672, col, flip2);
    }
  }
  ...
  public override die(): void {
    ...
    for (let i = 0; i < count; i++) {
      this.level.add(new ItemEntity(new ResourceItem(Resources.rawchicken), this.x + Math.floor(Math.random() * 11) - 5, this.y + Math.floor(Math.random() * 11) - 5));
      this.level.add(new ItemEntity(new ResourceItem(Resources.feather),    this.x + Math.floor(Math.random() * 11) - 5, this.y + Math.floor(Math.random() * 11) - 5));
    }
