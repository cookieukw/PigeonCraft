...
export class Zombie extends Mob {
  ...
  public override tick(input: Input): void {
    ...
    let speed = this.tickTime & 1;
    if (!this.move(this.xa * speed, this.ya * speed) || Math.random() < 0.005) {
  ...
  public override render(screen: Screen): void {
    ...
    let col = Color.get(-1, 10, 252, 40); // Zombie green
    ...
    const tileBase = 448; // Mobs start here in this sheet
  ...
  public override die(): void {
    super.die();
    if (this.level) {
        this.level.add(new ItemEntity(new ResourceItem(Resources.cloth), this.x, this.y));
    }
  }
}
